 ...
 * desugar destructuring if needed
 *
 * @param {Object} node
 * @this {ast-types.Path}
 */
    ...
    if (n.VariableDeclaration.check(node)) {
            ...
            function (declarations, decl) {
                ...
                if(isPattern(id)) {
                    pushAll(declarations, 
                            createVariableDeclarationsFromPattern(decl.id, decl.init, scope));   
                } else {
        ...
        node.params = node.params.map(function(param) {
            if (isPattern(param)) {
                ...
                pushAll(declarations, createVariableDeclarationsFromPattern(param, id, scope));
                return id;
            } 
            return param;
        });
        ...
        if (isPattern(node.left)) {
            this.replace(
                b.variableDeclaration(
                    'var', 
                    createVariableDeclarationsFromPattern(node.left, node.right, scope)
                )
            );
        }
    } else {
...
/**
 * from a given destructuring pattern, and associated value expression
 * create variable declarations that desugar these pattern
 * 
 * @param {Node} pattern the destructuring pattern
...
/**
 * Transform an Esprima AST generated by desugaring all destructuring expression
 *
